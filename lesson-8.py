...
class Matrix:
    ...
    def __add__(self, other):
        for i in range(len(self.mx_list)):
            for i_2 in range(len(other.mx_list[i])):
                self.mx_list[i][i_2] = self.mx_list[i][i_2] + other.mx_list[i][i_2]
        return Matrix.__str__(self)


m = Matrix([[3, 2, 1], [-1, 2, 5], [4, -5, 3], [2, 4, -1]])
m2 = Matrix([[3, 6, 0], [0, -4, 3], [-3, 1, 0], [1, 0, 5]])
print(m.__add__(m2))
...
coat = Coat(7)
costume = Costume(5)
print(f"Расход ткани на {coat} - {coat.count:>5.02f} кв.метров")
print(f"Расход ткани на {costume} - {costume.count:>5.02f} кв.метров")
print(f"Суммарный расход ткани :  {coat.count + costume.count:>5.02f} кв.метров")
...
# вернет строку: *****\n*****\n*****.
# использовать не __truediv__, а __floordiv__ - целочисленное деление.
...
class Cell:
    ...
    def __add__(self, other):
        return f'Сумма клеток составит: {self.quantity + other.quantity}'

    def __sub__(self, other):
        sub = self.quantity - other.quantity
        return f'Разность клеток составит: {sub}' if sub > 0 else f'Разница клеток отрицательна: {sub}'

    def __floordiv__(self, other):
        div = self.quantity // other.quantity
        div2 = other.quantity // self.quantity
        if div > 0 and div < 2:
            return f'1я клетка больше 2й'
        elif div >= 2:
            return f'1я клетка более, чем в {div} раза больше 2й'
        elif div2 > 0 and div2 < 2:
            return f'2я клетка больше 1й'
        else:
            return f'2я клетка более, чем в {div2} раза больше 1й'

    def __mul__(self, other):
        return f'Размножение клеток даст результат: {self.quantity * other.quantity}'
